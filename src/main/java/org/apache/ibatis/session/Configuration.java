...
public class Configuration {
    ...
    protected boolean returnInstanceForEmptyRow; //当返回行的所有列都是空时，MyBatis默认返回 null。 当开启这个设置时，MyBatis会返回一个空实例。 请注意，它也适用于嵌套的结果集 （如集合或关联）。（新增于 3.4.2）

    protected String logPrefix; //指定 MyBatis 增加到日志名称的前缀
    protected Class<? extends Log> logImpl; //指定 MyBatis 所用日志的具体实现，未指定时将自动查找。
    protected Class<? extends VFS> vfsImpl; //指定 VFS 的实现
    /**
     * MyBatis 利用本地缓存机制（Local Cache）防止循环引用（circular references）和加速重复嵌套查询。
     * 默认值为 SESSION，这种情况下会缓存一个会话中执行的所有查询。 若设置值为 STATEMENT，本地会话仅用在语句执行上，对相同 SqlSession 的不同调用将不会共享数据
     */
    protected LocalCacheScope localCacheScope = LocalCacheScope.SESSION;
    protected JdbcType jdbcTypeForNull = JdbcType.OTHER; //当没有为参数提供特定的 JDBC 类型时，为空值指定 JDBC 类型。 某些驱动需要指定列的 JDBC 类型，多数情况直接用一般类型即可，比如 NULL、VARCHAR 或 OTHER
    protected Set<String> lazyLoadTriggerMethods = new HashSet<>(Arrays.asList("equals", "clone", "hashCode", "toString")); //指定哪个对象的方法触发一次延迟加载
    ...
    protected boolean lazyLoadingEnabled = false;  //延迟加载的全局开关。当开启时，所有关联对象都会延迟加载。 特定关联关系中可通过设置 fetchType 属性来覆盖该项的开关状态
     ...
     *
     * map保存cache-ref关系。键是引用绑定到另一个缓存的命名空间的命名空间，值是实际缓存绑定到的命名空间
     */
